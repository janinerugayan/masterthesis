...
def score_word_token_boundaries(ref, seg, tolerance=0):
    ...
    for i_boundary, boundary_ref in enumerate(ref):
        ...
        n_boundaries_seg += len(seg_intervals)
...
def eval_segmentation(cfg):

    # Directories
    seg_dir = (
        Path(utils.to_absolute_path("exp"))/cfg.model/cfg.dataset/cfg.split/
        cfg.seg_tag/"intervals"
        )
    phone_ref_dir = (
        Path(utils.to_absolute_path("data"))/cfg.dataset/"phone_intervals"
        )
    word_ref_dir = (
        Path(utils.to_absolute_path("data"))/cfg.dataset/"word_intervals"
        )
    ...
    p, r, f = score_boundaries(
        reference_list, segmentation_list, tolerance=cfg.phone_tolerance
        )
    print("-"*(79 - 4))
    ...
    if word_ref_dir.is_dir():
        ...
        p, r, f = score_boundaries(
            reference_list, segmentation_list, tolerance=cfg.word_tolerance
            )
        ...
        print("Word token boundaries (have to double check):")
        print("Precision: {:.2f}%".format(p*100))
        print("Recall: {:.2f}%".format(r*100))
        print("F-score: {:.2f}%".format(f*100))
        print("OS: {:.2f}%".format(get_os(p, r)*100))
