...
def l2_segmentation(embedding, z, output_path, utt_key, n_min_frames=0, n_max_frames=15, dur_weight=20**2):
        ...
        for j in np.where(boundaries)[0]:
            ...
            code = np.argmin(distances)  # original: argmin, try to use argmax??
...
#-----------------------------------------------------------------------------#
#                        L2 SEGMENTATION - my version                         #
#-----------------------------------------------------------------------------#

def custom_viterbi_myver(costs, n_frames):
    """
    Viterbi segmentation of an utterance of length `n_frames` based on `costs`.
...
def l2_segmentation_myver(embedding, z, output_path, utt_key, n_min_frames=0, n_max_frames=15, dur_weight=20**2):
        ...
        # Viterbi segmentation
        summed_cost, boundaries = custom_viterbi_myver(costs, z.shape[0])
        ...
        for j in np.where(boundaries)[0]:
            ...
            distances = np.sum(embedding_distances[i_start:i_end, :], axis=0)
            distances_record.append(distances)
            code = np.argmax(distances)  # original: argmin, try to use argmax??
